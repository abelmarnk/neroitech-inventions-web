...
interface TaskDraft extends Omit<Task, "completed"> {
  completed?: boolean;
}
...
export const CreateQuestModal: React.FC<CreateQuestModalProps> = ({
  ...
  const addTask = () =>
    setTasks((t) => [
      ...t,
      { id: `${Date.now()}-${t.length + 1}`, title: "", description: "" },
    ]);
  ...
  const generateSummary = useCallback((quest: Quest) => {
    return [
      ...
      "Tasks:",
      ...quest.tasks.map(
        (t: Task, i: number) => `  ${i + 1}. ${t.title} - ${t.description}`
      ),
    ].join("\n");
  }, []);
  ...
  return (
    <BaseModal open={open} onClose={onClose} title="Create a Quest">
      <h3>Quest Options</h3>
      <p>
        Select the engagement types and quantities for your quest. Minimum price
        is <span id="minPriceText">₦20</span> per engagement.
      </p>
      ...
      <form>
        ...
        <div className="price-calculation">
          <h3>
            Total Price:{" "}
            <span id="totalPrice">{currency === "NGN" ? "₦0" : "0 USDC"}</span>
          </h3>
        </div>
        ...
        <div className="buttons-container">
          ...
          <button
            type="button"
            className="modal-close-btn"
            onClick={() => window.open("https://t.me/SnappQuest/146", "_blank")}
          >
            Custom Quest
          </button>
        </div>
